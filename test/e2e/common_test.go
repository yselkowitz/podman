...
func (p *PodmanTestIntegration) Cleanup() {
	...
	Expect(stop).To(Exit(0), "command: %v", stop.Command.Args)
	Expect(podrm).To(Exit(0), "command: %v", podrm.Command.Args)
	Expect(rmall).To(Exit(0), "command: %v", rmall.Command.Args)
}
